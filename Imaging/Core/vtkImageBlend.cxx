  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (this->GetNumberOfInputConnections(0) == 1)
  {
    ...
    vtkImageData* inData = static_cast<vtkImageData*>(info->Get(vtkDataObject::DATA_OBJECT()));
  ...
  vtkImageData* outData, T*, double opacity, bool blendAlpha, int id)
{
  ...
  if (inData->GetScalarType() == VTK_DOUBLE || inData->GetScalarType() == VTK_FLOAT)
  {
    minA = 0.0;
    maxA = 1.0;
  }
...
void vtkImageBlendCompoundExecute(vtkImageBlend* self, int extent[6], vtkImageData* inData, T*,
  vtkImageData* tmpData, vtkImageData* weightSum, double opacity, double threshold)
{
  // Opacity
  double minA, maxA;
  double r;
  ...
  T*, vtkImageData* tmpData, vtkImageData* tmpSumData, vtkTypeBool compoundAlpha)
{
  ...
  double minA, maxA;
  if (outData->GetScalarType() == VTK_DOUBLE || outData->GetScalarType() == VTK_FLOAT)
  {
    minA = 0.0;
    maxA = 1.0;
  }
  ...
  vtkImageData*** inData, vtkImageData** outData, int outExt[6], int id)
{
  ...
  for (int idx1 = first_index; idx1 < this->GetNumberOfInputConnections(0); ++idx1)
  {
    if (inData[0][idx1] != nullptr)
    {
      ...
      // vtkDebugMacro("Execute: " << idx1 << "=>" << extent[0] << ", " << extent[1] << " / " <<
      ...
      switch (this->BlendMode)
      {
        ...
        case VTK_IMAGE_BLEND_MODE_COMPOUND:
          switch (inData[0][idx1]->GetScalarType())
          {
            vtkTemplateMacro(vtkImageBlendCompoundExecute(this, extent, inData[0][idx1],
              static_cast<VTK_TT*>(inPtr), tmpData, alphaWeightSum, opacity,
  ...
  switch (this->BlendMode)
  {
    ...
    case VTK_IMAGE_BLEND_MODE_COMPOUND:
      ...
      {
        vtkTemplateMacro(vtkImageBlendCompoundTransferExecute(this, outExt, outData[0],
          static_cast<VTK_TT*>(outPtr), tmpData, alphaWeightSum, this->CompoundAlpha));
        default:
          vtkErrorMacro(<< "Execute: Unknown ScalarType");
          return;
      }
